int getFirstDigit(String line) {
    int firstDigit = 0; // Initalisize firstDigit (Digit = Ziffer).
    char[] getParts = line.toCharArray(); // Create char-Array which contains each part of the line's word.
    for (int i = 0; i <= getParts.length - 1; i++) { // Iterates char-Array. As a result we are iterating through each part/symbol of the word in the line.
        if (Character.isDigit(getParts[i])) { // If the part/symbol is a digit:
...
// Testing
assert getFirstDigit("1abc2") == 1;
assert getFirstDigit("pqr3stu8vwx") == 3;
assert getFirstDigit("a1b2c3d4e5f") == 1;
assert getFirstDigit("treb7uchet") == 7;



int getLastDigit(String line) {
    int lastDigit = 0;
    char[] getParts = line.toCharArray();
    for (int i = getParts.length - 1; i >= 0; i--) { // Iterates char-Array backwards.
        if (Character.isDigit(getParts[i])) {
            lastDigit = Character.getNumericValue(getParts[i]);
            break;
        }
    }
    return lastDigit;
...
// Testing
assert getLastDigit("1abc2") == 2;
assert getLastDigit("pqr3stu8vwx") == 8;
assert getLastDigit("a1b2c3d4e5f") == 5;
assert getLastDigit("treb7uchet") == 7;



int[] formSingleTwoDigitNumbers(String[] lines) {
    int[] singleTwoDigitNumbersArray = new int[lines.length];
    int counter = 0;
    for (int i = 0; i <= lines.length - 1; i++) {
        int firstDigit = getFirstDigit(lines[i]);
        int lastDigit = getLastDigit(lines[i]);
        firstDigit *= 10;
        int singleTwoDigitNumber = firstDigit + lastDigit;
        singleTwoDigitNumbersArray[counter++] = singleTwoDigitNumber;
    }
    return singleTwoDigitNumbersArray;
}

// Testing
assert Arrays.equals(formSingleTwoDigitNumbers(new String[] {"1abc2", "pqr3stu8vwx", "a1b2c3d4e5f", "treb7uchet"}), new int[] {12, 38, 15, 77});



int sumUpCalibrationValues(int[] twoDigitNumbers) {
...
// Testing
assert sumUpCalibrationValues(new int[] {12, 38, 15, 77}) == 142;
...
String[] puzzleInputArray = puzzleInput.split("\n");